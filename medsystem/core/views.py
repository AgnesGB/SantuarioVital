...
def alterar_tipo_usuario(request, usuario_id):
    ...
    if novo_tipo in ["MED", "OUT", "ADM"]:
        ...
        messages.success(
            request,
            f"Tipo de usuário de {usuario.nickname} alterado para {usuario.get_tipo_display()}.",
        )
    else:
...
class PacienteListView(MedicoRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = (
            super().get_queryset().select_related("cidade").order_by("nome")
...
class AnotacaoListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if busca:
            queryset = queryset.filter(
                Q(titulo__icontains=busca) | Q(conteudo__icontains=busca) | Q(tags__icontains=busca)
            )
        ...
        if data:
            try:
                ...
                queryset = queryset.filter(
                    Q(data_criacao__date=date_obj) | Q(data_atualizacao__date=date_obj)
                )
            except ValueError:
                pass
...
def recuperar_senha(request):
    ...
    if request.method == "POST":
        ...
        if attempts >= 5:
            messages.error(
                request, "Você excedeu o limite de tentativas. Tente novamente em 1 hora."
            )
...
class RemedioDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["ingredientes_remedio"] = RemedioIngrediente.objects.filter(
            remedio=self.object
...
class RemedioCreateView(MedicoRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            context["ingredientes_formset"] = RemedioIngredienteFormSet(
