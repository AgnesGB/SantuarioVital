...
class Doenca(models.Model):
    ...
    sintomas = models.TextField(
        "Sintomas", blank=True, help_text="Descreva os sintomas, separados por vírgula"
    )
...
class Raca(models.Model):
    ...
    longevidade = models.CharField(
        max_length=200, blank=True, verbose_name="Longevidade", help_text="Ex: 80-100 anos"
    )
    ...
    pressao_arterial = models.CharField(
        max_length=200, blank=True, verbose_name="Pressão arterial", help_text="Ex: 120/80 mmHg"
    )
    bpm = models.CharField(
        max_length=200, blank=True, verbose_name="Batimentos por minuto", help_text="Ex: 60-100 bpm"
    )
...
class Diagnostico(models.Model):
    ...
    remedios = models.ManyToManyField(
        "Remedio", blank=True, related_name="diagnosticos", verbose_name="Remédios prescritos"
    )
...
class Besta(models.Model):
    ...
    doenca_relacionada = models.ManyToManyField(Doenca, related_name="bestas")
...
class AnotacaoPessoal(models.Model):
    ...
    tags = models.CharField(
        max_length=255, blank=True, help_text="Palavras-chave separadas por vírgula"
    )
...
class Ingrediente(models.Model):
    ...
    o_que_faz = models.TextField(
        verbose_name="O que faz", help_text="Propriedades e efeitos do ingrediente"
    )
    contra_indicacoes = models.TextField(
        blank=True, verbose_name="Contraindicações", help_text="Situações em que não deve ser usado"
    )
    reacoes_adversas = models.TextField(
        blank=True, verbose_name="Reações adversas", help_text="Possíveis efeitos colaterais"
    )
    cuidados_ao_uso = models.TextField(
        blank=True, verbose_name="Cuidados ao uso", help_text="Precauções necessárias"
    )
...
class Remedio(models.Model):
    ...
    descricao = models.TextField(
        blank=True, verbose_name="Descrição", help_text="Descrição geral do remédio"
    )
    modo_uso = models.TextField(
        blank=True, verbose_name="Modo de uso", help_text="Instruções de como usar"
    )
    ingredientes = models.ManyToManyField(
        Ingrediente, through="RemedioIngrediente", related_name="remedios"
    )
...
class RemedioIngrediente(models.Model):
    ...
    quantidade = models.CharField(
        max_length=200, verbose_name="Quantidade", help_text="Ex: 2 colheres, 100g, 5 gotas"
    )
...
def validar_ingrediente_remedio(ingrediente, remedio):
    """Validates if an ingredient is compatible with a remedy."""
